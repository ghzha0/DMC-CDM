...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, device, mean_type, noise_schedule, noise_scale, noise_min, noise_max,\
    ...
    def calculate_for_diffusion(self):
        ...
        self.alphas_cumprod_next = torch.cat([self.alphas_cumprod[1:], torch.tensor([0.0]).to(self.device)]) # alpha_{t+1}
    ...
    def p_sample(self, input, steps, sampling_noise=False):
        tgt_log_seqs, tgt_answers, ctx_log_seqs, ctx_answers = input['tgt_seq_logs'], input['tgt_answers'], input['ctx_seq_logs'], input['ctx_answers']
        x_start = self.model.get_user_embedding(tgt_log_seqs, tgt_answers)
        ...
        if self.noise_scale == 0.:
            for i in indices:
                ...
                x_t = self.model(x_t, t, ctx_log_seqs, ctx_answers, False)
            return x_t

        for i in indices:
            t = torch.tensor([i] * x_t.shape[0]).to(x_start.device)
            out = self.p_mean_variance(x_t, t, ctx_log_seqs, ctx_answers)
            if sampling_noise:
    ...
    def train_losses(self, input, reweight=True):
        terms = {}
        tgt_log_seqs, tgt_answers, ctx_log_seqs, ctx_answers = input['tgt_seq_logs'], input['tgt_answers'], input['ctx_seq_logs'], input['ctx_answers']
        with torch.no_grad():
            x_start = self.model.get_user_embedding(tgt_log_seqs, tgt_answers)
        ...
        if random.random() > 0.9: # set ctx info invalid
            classifer_free_guidance = True
        else:
            classifer_free_guidance = False

        model_output = self.model(x_t, ts, ctx_log_seqs, ctx_answers, classifer_free_guidance)
        ...
        if reweight == True:
            ...
            weight = torch.tensor([1.0] * len(target)).to(x_start.device)
    ...
    def sample_timesteps(self, batch_size, device, method='uniform', uniform_prob=0.001):
        if method == 'importance':  # importance sampling
            ...
            t = torch.multinomial(pt_all, num_samples=batch_size, replacement=True).to(device).long()
    ...
    def p_mean_variance(self, x, t, ctx_log_seqs, ctx_answers):
        """
        ...
        B, C = x.shape[:2]
        assert t.shape == (B, )
        model_output = self.model(x, t, ctx_log_seqs, ctx_answers, False)
    ...
    def forward(self, input, steps):
        ...
        x_start = self.model.get_user_embedding(tgt_log_seqs, tgt_answers)
        noise = torch.randn_like(x_start)
        ts = torch.tensor([steps - 1] * x_start.shape[0]).to(x_start.device)
        x_t = self.q_sample(x_start, ts, noise)

        model_output = self.model(x_t, ts, ctx_log_seqs, ctx_answers, False)
        return model_output
